...
const fileFilter = (req, file, cb) => {
    const allowedFileTypes = ["application/pdf", "application/msword", "image/jpeg", "image/png"];
...
router.use((err, req, res, next) => {
    if (err instanceof multer.MulterError) {
        ...
        return res.status(400).json({ message: "Incorrect file type. Allowed types are PDF, DOC, DOCX, JPEG, PNG." });
    }
